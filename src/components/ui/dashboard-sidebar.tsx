...
export function AdminLayoutShell({ children, currentPath = '/admin' }: { children: React.ReactNode; currentPath?: string }) {
  const [isOpen, setIsOpen] = useState(true);
  const [activeWorkspace, setActiveWorkspace] = useState('Aakriti');
  const [isSearchOpen, setIsSearchOpen] = useState(false);

  // Derive active ID from path if possible, or use a default
  const [activeId, setActiveId] = useState('home');

  React.useEffect(() => {
    if (currentPath.includes('bookings')) setActiveId('projects');
    else if (currentPath.includes('calendar')) setActiveId('calendar');
    else if (currentPath.includes('artists') || currentPath.includes('customers')) setActiveId('team');
    ...
    <div className="flex flex-col items-center justify-center w-full h-screen bg-background font-sans">
      ...
      <div className="relative w-full h-full bg-card flex overflow-hidden">
        ...
        <div className="flex-1 bg-black/[0.02] dark:bg-white/[0.02] flex flex-col min-w-0 transition-all duration-300">
           ...
           <div className="p-6 md:p-8 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] h-full relative">
             {children}
           </div>
        </div>
